...
withDefaults(defineProps<{ question: Question; withSpeaker?: boolean }>(), { withSpeaker: false })
</script>
  ...
  <div class="stem">
    <template v-for="(part, i) in question.stem" :key="i">
      ...
      <div v-else-if="part.kind === 'expr'" class="stem-expr" :class="{ long: part.expr.length > 12 }">
        <span v-if="withSpeaker && i === 0" class="speaker" aria-hidden="true">🔊</span>{{ part.expr }}
      </div>
      <TenFrame
      ...
      <RulerGauge
        ...
        :to="part.to"
      />
      <VerticalForm v-else-if="part.kind === 'vertical'" :a="part.a" :op="part.op" :b="part.b" />
