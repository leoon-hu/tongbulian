...
// 结果页（B9）的三种按钮组合：单设备（再来一局 / 换个游戏 / 退出）、线上主持人（下一章最大 + 再来一局 + 退出；本册最后一个知识点写「打完啦」）、线上非主持人（等主持人）
...
describe('结果页按钮（B9）', () => {
  ...
  it('线上主持人：下一章最大（绿）+ 再来一局（蓝）+ 退出，上面写着下一章的知识点；点了发 next', async () => {
    const next = nextKp(KP)!
    const w = mount(ResultPanel, { props: { state: ended(), changeable: false, next } })
    const btns = w.findAll('.big-btn')
    expect(btns).toHaveLength(3)
  ...
  it('线上主持人在本册最后一个知识点：没有下一章、写「这一册都打完啦」，再来一局回到绿色', () => {
    expect(nextKp('s1-05-carry-add')).toBeNull() // 上册最后一个知识点
    const w = mount(ResultPanel, { props: { state: ended(), changeable: false, next: null } })
    expect(w.find('.next-btn').exists()).toBe(false)
  ...
  it('线上非主持人：只有退出，写「等主持人开始下一局」', () => {
    const w = mount(ResultPanel, { props: { state: ended(), host: false, changeable: false, next: nextKp(KP) } })
    expect(w.findAll('.big-btn')).toHaveLength(1)
    expect(w.find('.next-btn').exists()).toBe(false)
    expect(shown(w.find('.host-wait'))).toContain('等主持人开始下一局')
    setLang('en')
  })
})
