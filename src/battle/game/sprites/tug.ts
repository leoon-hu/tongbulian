...
export function drawPuller(ctx: CanvasRenderingContext2D, x: number, y: number, s: number, kind: CritterKind, team: Team, p: PullerPose): void {
  ...
  withTransform(ctx, x, y - p.lift + fallen * s * 0.3, fallen * 0.55 * p.facing, p.facing, 1, () => {
    ...
    const shY = -0.74 * s
    const headX = -lean * 0.48 * s
    const headY = -0.98 * s
    ctx.lineCap = 'round'
    // 腿：前脚蹬地、后脚撑着；摔倒时前腿翘起来
    ctx.strokeStyle = fur
    ctx.lineWidth = Math.max(1.5, s * 0.14)
    ctx.beginPath()
    ctx.moveTo(hipX, hipY)
    if (fallen > 0) ctx.lineTo(0.42 * s, -0.25 * s * fallen)
    else ctx.lineTo((0.26 + lean * 0.1) * s, 0)
    ctx.moveTo(hipX, hipY)
    ctx.lineTo((-0.24 - lean * 0.14) * s, 0)
    ctx.stroke()
    // 背心
    ...
    if (p.cheer > 0 || fallen > 0) {
      ...
      ctx.lineTo(0.5 * s, p.handY)
      ctx.moveTo(shX + 0.06 * s, shY + 0.12 * s)
      ctx.lineTo(0.56 * s, p.handY + 0.04 * s)
    }
    ctx.stroke()
    // 头
    withTransform(ctx, headX, headY, -lean * 0.25, 1, 1, () => {
      ctx.fillStyle = fur
      ...
      const ey = -0.05 * s
      const squint = p.blink > 0.5 || (p.strain > 0.6 && fallen === 0)
      for (const ex of [-0.03 * s, 0.13 * s]) {
        if (squint) ctx.fillRect(ex - 0.035 * s, ey, 0.07 * s, Math.max(1, 0.02 * s))
        else {
          circle(ctx, ex, ey, fallen > 0.3 ? 0.045 * s : 0.035 * s)
          ctx.fill()
        }
      }
      // 嘴
      ...
      ctx.beginPath()
      if (fallen > 0.3) ctx.arc(0.07 * s, 0.11 * s, 0.035 * s, 0, Math.PI * 2)
...
/** 抓绳的两只手（画在绳子上面） */
export function drawHands(ctx: CanvasRenderingContext2D, x: number, y: number, s: number, kind: CritterKind, facing: 1 | -1): void {
  ctx.fillStyle = FUR[kind][0]
  circle(ctx, x, y, s * 0.09)
  ctx.fill()
  circle(ctx, x + facing * s * 0.07, y + s * 0.035, s * 0.08)
  ctx.fill()
}
