/**
 * 种花（需求 B36m）：把模型与渲染组装成 GameModule。
...
export function createFlowerGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: FlowerModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new FlowerModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
