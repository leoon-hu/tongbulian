/**
 * 拆城堡（需求 B36u）：把模型与渲染组装成 GameModule。
...
import { CastleModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createCastleGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: CastleModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new CastleModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
