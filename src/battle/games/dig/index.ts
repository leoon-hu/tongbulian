/**
 * 挖宝（需求 B36k）：把模型与渲染组装成 GameModule。
...
import { DigModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createDigGame(): GameModule {
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new DigModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
