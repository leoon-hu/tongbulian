/**
 * 盖楼（需求 B36c）：把模型与渲染组装成 GameModule。
...
import { TowerModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createTowerGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: TowerModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new TowerModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
