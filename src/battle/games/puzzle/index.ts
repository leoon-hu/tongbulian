/**
 * 拼图（需求 B36r）：把模型与渲染组装成 GameModule。
...
export function createPuzzleGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: PuzzleModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new PuzzleModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
