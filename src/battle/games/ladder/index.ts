/**
 * 爬梯子（需求 B36j）：把模型与渲染组装成 GameModule。
...
import { LadderModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createLadderGame(): GameModule {
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new LadderModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
