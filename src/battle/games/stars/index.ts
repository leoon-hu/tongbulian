/**
 * 点亮星星（需求 B36q）：把模型与渲染组装成 GameModule。
...
import { StarsModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createStarsGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: StarsModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new StarsModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
