/**
 * 开火车（需求 B36v）：把模型与渲染组装成 GameModule。
...
export function createTrainGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: TrainModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new TrainModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
