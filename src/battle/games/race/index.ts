/**
 * 龟兔赛跑（需求 B36a）：把模型与渲染组装成 GameModule。
...
import { RaceModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createRaceGame(): GameModule {
  ...
  function makeBackground(): void {
    ...
    try {
      ...
      if (!octx) return
      renderBackground(octx, model.geo, model.target)
      bg = off
      bgTarget = model.target
    } catch {
      bg = null
    }
  }
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new RaceModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
    setState(s: GameState) {
      if (!model) return
      model.setState(s)
      if (bg && bgTarget !== model.target) makeBackground()
    },
    onEvent(e) {
      model?.onEvent(e)
    },
    ...
    tick(dt) {
      ...
      else renderBackground(ctx, model.geo, model.target)
