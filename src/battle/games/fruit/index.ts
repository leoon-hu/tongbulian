/**
 * 摘果子（需求 B36p）：把模型与渲染组装成 GameModule。
...
export function createFruitGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: FruitModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new FruitModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
