/**
 * 跷跷板（需求 B36s）：把模型与渲染组装成 GameModule。
...
import { SeesawModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createSeesawGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: SeesawModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new SeesawModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
