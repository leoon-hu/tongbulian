/**
 * 热气球（需求 B36h）：把模型与渲染组装成 GameModule。
...
import { BalloonModel } from './model'
import { renderBackground, renderDynamic } from './render'

export function createBalloonGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: BalloonModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new BalloonModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
