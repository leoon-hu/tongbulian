/**
 * 吹泡泡（需求 B36o）：把模型与渲染组装成 GameModule。
...
export function createBubbleGame(): GameModule {
  ...
  let compact = false
  let quality = 0
  let model: BubbleModel | null = null
  ...
  return {
    ...
    mount(host: GameHostInfo) {
      ...
      model = new BubbleModel(createRng(), { reducedMotion: host.reducedMotion })
      relayout()
    },
