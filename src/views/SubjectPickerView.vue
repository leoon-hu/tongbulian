...
// 标题上方一条对战 hero（F1，2026-09-21 对战版定位）：每种游戏的图标 + 一句带拼音的话，静态、不出声、不可点。
// 卡片下面给家长（也给搜索引擎）的说明、帮助入口与各上线课程的知识点清单链接：清单是构建时生成的静态页
// <学科>/<年级>/（见 src/seo），普通链接、不走路由。
// 再下面是页脚（F1，排版三个静态站统一）：版本卡片（当前版本 + 检查更新，AppVersion）→ 一句「开源」说明（免费 / 无广告 /
// 不收集个人信息 / 代码开源）→ 三个给家长的动作：GitHub 源码（新窗口）、分享给朋友（系统分享面板或复制一段话）、
// 联系站长（弹站长微信二维码）→「更多应用」链到同一作者的另外三个站（标题单独一行，手机上一个站一行）。
// 都是家长自己点开的，不出声不注音。
...
<template>
  <div class="picker">
    ...
    <footer class="foot">
      <AppVersion />
      <p class="foot-open">{{ ui('open.claim') }}</p>
      ...
      <nav class="foot-sites" :aria-label="ui('sites.more')">
        <span class="foot-sites-title">{{ ui('sites.more') }}</span>
...
/* 页脚（三个静态站同一套排版）：和上面的内容用一条细线隔开，版本卡片在最上面 */
.foot {
  display: flex;
  flex-direction: column;
  align-items: center;
...
/* 更多应用：标题单独一行，站点宽屏排一行、手机上一个站一行（不再出现「标题 + 第一个站」挤一行、后两个各占一行） */
.foot-sites {
  display: flex;
  flex-wrap: wrap;
  justify-content: center;
