...
- **各用各的（多设备房间）**：主持人在设置页选「各用各的」→「建房间」，得到一个 6 位房间号和三个链接（红队 / 蓝队 / 观战），每个都能复制、系统分享或显示成二维码（本机生成），另一台设备扫码或输房间号就进来；打开链接的人默认在链接指定的队，大厅里可以换队，主持人可以「锁定队伍」。大厅显示知识点、游戏（主持人可换）、这个游戏的规则句、两队名单（每队最多 6 人，人数不要求相等，谁是主持人、谁掉线了都标着）、观战的人；每个参赛者点「准备好了」，自己的难度档自己选（主持人可以改任何人的），两队都有人且都准备好了主持人才能「开始比赛」。开始时刻由服务器定，各台设备一起倒数；比赛时每台设备上都是完整的竞技场，**只有自己那一行能操作**，别人的行实时显示他在答什么、按了什么、对还是错，队伍比分 = 队里所有人答对的题数之和。题目在各自设备上生成与判分（同一个种子，服务器只记谁答到第几题、对了几题），断线自动重连并接回原座位，同一个浏览器再开一个标签页会顶掉先开的；主持人掉线主持权自动给最早在线的人。结果页只有主持人有「再来一局 / 换个游戏」，别人等着。房间只在服务器内存里（没人 10 分钟或建房 3 小时后销毁），不落库、不记内容日志；地址栏 `#/battle/<房间号>?t=red|blue|watch`，房间号本身就是密钥。用 `file://` 打开或托管方没有中继服务时这一项置灰。
...
- **对战怎么玩**：地图右上角打开「⚔️ 对战」再点知识点（或练习页页头的 ⚔️）→ 选「打机器人」（🐢 慢 / 🐰 中 / 🚀 快）、「两人一台」或「各用各的」（建房间发链接 / 扫码）→ 选游戏（8 种 + 🎲 随机）→ 第一次问一次名字 → 先讲一句这个游戏的规则 → 预备、3、2、1、开始。竞技场一律横向：左红右蓝，游戏画面在上方横条或中间竖条，每人一行题目 + 键盘 / 选项卡。结束进结果页：再来一局 / 换个游戏 / 退出。
...
- **游戏技巧**：先听清题再答（🔊 再听一遍）；数字键盘打错用 ⌫、确认再按 ✓，选项卡先看拼音；打机器人先选 🐢 慢，赢了再升；稳比快重要，连对有加成；两人一台各管各的；各用各的先都进大厅准备好再开始、掉线别关页面会自动接回；先在练习模式做熟再对战；装到主屏幕全屏、离线都能玩。
- **常见问题**：没声音（看 🔇、iPhone 静音键、先点一下屏幕）、手机要横屏、怎么装到主屏幕离线用、「各用各的」连不上或进不去房间（要网络和对战服务、房间号 / 版本 / 顶号 / 比赛已开始只能观战）、进度只在本机不同步、题目几乎不重复、「火箭升空 3D」是要联网首次下载的试点、机器人有三档且会答错、改名字在对战设置页、其它年级和学科陆续补充、免费无广告开源。
...
├── views/                   选学科 → 选年级 → 知识点地图 → 练习；battle/ 对战设置页、单设备竞技场页、多设备房间页（大厅 → 竞技场 → 结果）
...
数据层级：学科 → 年级 → 课程（内容包）→ 上册 / 下册 → 单元 → 知识点 → 生成器。路由 `/s/:subjectId/g/:gradeId/practice/:kpId`；对战只带知识点：`/battle/new/:kpId`（设置）、`/battle/local/:kpId?mode=ai|duo`（单设备竞技场）、`/battle/:code`（多设备房间：大厅 → 竞技场 → 结果，`?t=red|blue|watch` 只是进来时的默认身份）。
