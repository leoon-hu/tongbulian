...
- **结果页**：🏆 谁赢、比分、用时、每人「答 n · 对 m」，输的一方写「差一点点！」；「再来一局」（同样的人和游戏，换一组题）、「换个游戏」、「退出」；多设备房间里是「下一章」（最大，同一房间接着打本册下一个知识点，最后一个知识点写「这一册都打完啦！」）、「再来一局」、「不玩了」（关房间、大家一起回地图），三个角色都一样、谁先点算谁的。对战不改地图上的练习进度，也不存战绩。
...
- **对战怎么玩**：地图右上角打开「⚔️ 对战」再点知识点（或练习页页头的 ⚔️）→ 选「打机器人」（🐢 慢 / 🐰 中 / 🚀 快）、「两人一台」或「各用各的」（建房间出三个二维码和口令，红蓝各扫各的或输口令，人齐了自动开始）→ 游戏按章节排定（配置里可换，只算这一次）→ 第一次问一次名字 → 先讲一句这个游戏的规则 → 预备、3、2、1、开始。竞技场一律横向：左红右蓝，游戏画面在上方横条或中间竖条，每人一行题目 + 键盘 / 选项卡。结束进结果页：再来一局 / 换个游戏 / 退出（房间里是下一章 / 再来一局 / 不玩了，谁点都算）。
