...
- **识字不多也能自己玩**：每个汉字标拼音，每道题自动朗读（预先合成好的语音片段拼播，不依赖浏览器 TTS）；对战里让孩子照着做的提示（跟谁打、你叫什么、扫码进入、等大家进来、输口令……）在出现的那一刻也会读出来。
...
<p align="center">
  ...
  <img src="screenshots/battle-setup.png" width="24%" alt="跟谁打：自己练、打机器人、两人一台、各用各的（⚙️ 配置里有机器人快慢、选游戏、改名字）">
</p>
<p align="center"><sub>手机：首页选学科 → 知识点地图 → 点一个知识点就到「跟谁打」（自己练 + 三种对战）</sub></p>
...
2. 点一个知识点，直接到「跟谁打？」（带拼音，会读出来）：第一张「自己练」默认就选着，点「开始」开始练习，一轮 8 题，进题自动读题干，🔊 可以再听一遍；用**数字键盘**或**四选一卡片**作答；答对撒花、夸一句，答错读出并显示正确答案（有教具的知识点会演示一遍），点「我知道了」继续。一轮做完是结算页，和对战的结果页一样三个按钮：「下一章」（本册的下一个知识点）、「再练一次」、「不练了」。练习页排得紧凑（顶部栏在这一页缩小一号），常见手机竖屏上题目、教具和作答区一屏放得下，答错时连同讲解也不用往下滚。
3. 做完一轮就算「已完成」，地图上打勾。没做完的节点下面有 8 格进度条和这一轮的「x 题 · 对 y · 错 z」，中途退出下次接着做；做完的显示那一轮的对错数，再进去就开新的一轮重新计。家长也可以在地图上直接点状态标签手动标记 / 取消，不用做题。
4. 想比一比：点知识点后在「跟谁打」里选打机器人、两人一台或各用各的，开始。第一次默认选着「自己练」，之后默认选着上次开始时选的那张。
...
点知识点就到这一页：四张卡（自己练、打机器人、两人一台、各用各的，各画一幅示意图，一眼看出模式；手机竖屏上两排各两张）和「开始」，第一次默认选着「自己练」，之后默认选着上次开始时选的那张；机器人快慢、选游戏、改名字都在页头「⚙️ 配置」里。
