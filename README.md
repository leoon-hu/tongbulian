...
题目由程序按知识点**随机生成**（不是题库），每次练习 8 题，题干配十格阵、钟面、人民币、图形、数轴等可视化教具；答错当场用教具演示讲解（凑十法 / 破十法有动画）。**每个汉字都标拼音，每道题自动朗读**（离线预合成的语音，不依赖浏览器 TTS），识字不多的孩子也能自己练。还有**对战模式**：同一个知识点的题，两边比谁先答对 8 题——打机器人、两个人一台 iPad 左右分屏，或者每人一台设备扫码进同一个房间；每答对一题游戏画面就走一步（赛跑、火箭升空、盖楼、拔河、融冰……）。练习纯前端、不联网、不要账号，进度只存在本机浏览器里，`dist/index.html` 可以直接双击离线打开；只有对战的多设备模式需要一个小小的内存中继服务（仓库 `server/`）。界面中文 / 英文一键切换（题目、拼音、朗读都跟着切）。
...
同一个知识点的题，🔴 红队和 🔵 蓝队各答各的，**谁先答对 8 题谁赢**；答错不扣分、不锁题，1 秒多显示正确答案就出下一题。得分不只是数字：每答对一题，队区闪一下队色、飘一个「+1」、比分弹一下、队名下的 8 颗进度点亮一颗，游戏画面同时发生一次看得见的变化。本设备第一次进某个游戏时先讲一句规则（「对战开始啦！答对一题，火箭就升高一段，看谁先飞到星星，谁就赢了哦！」，每种游戏一句自己的话，说完再倒数；同一个游戏一天内不重复，再来一局也不讲），得分还配这种游戏自己的音效（赛车轰一下、火箭点火、盖楼砖落地咚、融冰咔嚓 + 水滴……），到 4 分弹「到一半啦！」，赢了播报队名后再接一句游戏话（「火箭飞到星星啦！」）；连对 2 题起有 🔥 ×n，「叮」的音高越来越高；连对 3 / 5 题弹「连对 n 题！」、从落后变领先弹「反超啦！」、到 7 分弹「还差一分！」（都朗读）；到 8 分全屏撒队色彩纸、小号声加欢呼。现在有五种「皮肤」，每种都是一个有背景、有一直在动的小装饰、有待机 / 得分 / 冲刺 / 胜利动作的小场景，选游戏时可以挑，也可以「🎲 随机」：

| 皮肤 | 位置 | 场景 | 得 1 分 | 到 8 分 |
|---|---|---|---|---|
| 🐢🐇 赛跑 | 上方横条 | 天空、太阳、飘云、草地跑道、格子终点线和观众 | 自己的动物向终点跑一格、扬起尘土；到 6 分终点旗挥得更快 | 冲线，观众跳起来 |
| 🏎️ 赛车 | 上方横条 | 柏油路、红白路肩、格子终点线、发令员和观众 | 自己的车向前跑一格、轮子转、冒尾气；连对喷氮气 | 冲过格子线翘头蹦，撒彩纸；输的熄火冒黑烟 |
| 🚀 火箭升空 | 左右之间 | 星空、闪的星星、行星，火箭平时悬浮喷小火 | 升高一段、喷大火；到 6 分目标星星脉动 | 到达星星，放烟花 |
| 🎈 热气球 | 左右之间 | 天空渐变、太阳、山丘、飘云，顶上一层厚云是目标，小鸟飞过 | 烧嘴爆燃冒火星、升高一段，篮子里的乘客挥手；连对烧得更旺 | 飞到云上再往上飘，撒彩纸；输的火熄了、球囊瘪一点往下沉 |
...
| 🧊 融冰 | 左右之间 | 极光、飘雪、远处的浮冰，底下是有波纹的水面、小鱼游来游去，两摞冰上各站一只系队色围巾的企鹅 | 对方最上面那块冰先裂再化掉、两侧淌水，对方的企鹅跟着往下沉；对方只剩一块时那块冰一闪一闪 | 对方冰全化，企鹅扑通掉进水里溅水花、仰面漂着蹬腿；自己的企鹅蹦着拍翅膀放雪花亮片，小鱼跳出水面 |

手机横屏的矮横条 / 窄竖条会用 CSS 容器查询自动隐藏装饰，只留角色与轨道。

五个游戏都已升级成 **canvas 实时绘图**（`src/battle/game/` 是与具体游戏无关的部分：契约、自研的 Canvas 2D 小引擎、宿主、保底画面；`src/battle/games/<id>/` 是每个游戏的模型与渲染）。已升级：🐢🐇 赛跑——天空、太阳、飘云、山丘与小树、两条草地跑道各 8 个蘑菇标记（跑过就亮）、举旗的发令员小猴、格子终点柱 + 彩旗拱门 + 4 只观众；乌龟和兔子是代码画的矢量角色（眨眼、待机呼吸、跑步腿摆 / 蹦跳压扁、耳朵后甩、扬尘、速度线），倒数原地蹦、开始落旗迸彩纸、连对加速、反超回头看、还差一分横幅发光、冲线喷彩纸、胜方转圈 / 后空翻、负方缩壳 / 耷拉耳朵。🚀 火箭升空——星空里二十几颗按各自节奏闪的星星、带环的行星、月亮、隔几秒划过的流星，两列各有发射台（平台 + 小塔）、细虚线导轨和顶上带光晕的目标星；火箭是矢量部件（尾翼、队色条纹与鼻锥、舷窗、抖动的尾焰），平时悬浮，得分喷大火向下冒烟升一段，连对火更大；倒数在台上抖、台下冒烟，开始喷火台面闪光，反超被超方晃一下，冲刺目标星脉动，抵达时星星炸成光芒、放三轮队色烟花、火箭绕小圈，负方断火冒黑烟微微下沉。🧱 盖楼——白天的天空、太阳、两层飘云、隔几秒飞过的小鸟、山丘、草地和两块地基；砖是队色圆角砖（底边阴影、顶边高光、按层交替的一扇宽窗 / 两扇窗），每座楼顶站一个矢量小工人（圆脸、队色安全帽、背心、锤子；待机抡锤、隔几秒眨眼），得分一块新砖从天上掉下来弹两下、扬尘、整座楼被压一下再弹回、工人跳上新楼顶，比分一次跳几层就逐块错开落，连对砖掉得更急；倒数工人在地基上蹦，开始举锤欢呼，反超被超的工人挠头看过去，冲刺楼顶上方一颗星星闪，封顶时屋顶从天上落下、队旗挥动、窗户从下往上亮成暖黄、放三轮队色烟花、工人跳上屋脊欢呼，负方工人在没盖完的楼顶坐下歇着。🪢 拔河——草地、太阳、飘云、山丘、两头的小树，正中一条白色虚线的中线和一个水坑，两头各两只观众小动物（左边红队粉丝、右边蓝队粉丝）；两队各两只矢量小动物（熊 + 猪对熊猫 + 猴）穿队色背心、系队色头带，双手抓着一根粗麻绳后仰用力（脚蹬地、屁股后坐、肩膀更靠后、用力时眯眼咬牙），绳子正中一个黄色蝴蝶结，**位置 = 比分差**：得分时整套人和绳子带一点过冲弹过去、蝴蝶结抖一下，被拉的一方踉跄前倾、脚下打滑扬尘，粉丝挥手；倒数四只原地蹦、绳子松垮，开始一起用力绳子绷直，反超被超的队冒汗，冲刺绳子绷紧发抖、观众一直挥手，还差一分蝴蝶结发光；结束时负方两只被拖过中线摔进水坑（溅水花、荡涟漪、坐在水里一脸惊讶），胜方松手举双手蹦着欢呼、放队色彩纸、观众跳起来，绳子落到地上。🧊 融冰——极地的淡蓝天空、顶上慢慢飘动的绿紫极光、飘落的雪花（大的六角、小的雪点）、远处的浮冰，底下是有波纹的水面和一条游来游去的小鱼；两摞 8 块冰砖立在水里，每摞顶上站着自己队的矢量企鹅（黑身白肚、橙嘴橙脚、眨眼、队色围巾），我得一分对方最上面那块冰先裂（裂纹长出来、冰块抖）再从上往下化掉、两侧淌水滴进水里，对方的企鹅跟着冰一起往下沉、落到下一块上颠一下，连对后化得更快；倒数在冰上原地蹦，开始拍翅膀，我得分我的企鹅拍翅膀、对方的晃一下，反超被超的企鹅皱眉冒汗，对方只剩一块冰时那块冰一闪一闪，冲刺雪下得更急；结束时负方最后一块冰化掉、企鹅扑通掉进水里（溅水花、荡涟漪）仰面漂着蹬腿一脸惊讶，胜方的企鹅蹦着拍翅膀、头顶连放三轮雪花亮片，小鱼高兴地跳出水面；水面是半透明的一层压在最上面，泡在水里的东西看得出淹着。升级后的游戏仍然只看比分、不认识题目，并且被严格隔离：只能填满竞技场分给它的那个盒子、不接触摸、不出声、没有文字也没有 CSS、按需加载、出错自动换成保底进度条，页面其余部分与布局不受任何游戏改动影响（有测试保证换任何皮肤盒子之外的 DOM 完全一致）。

第二批游戏（按同一架构逐个做）：🏎️ 赛车——柏油路上红白路肩、车道虚线、黑白格子终点线，队色赛车的座舱里探出司机的脑袋，两个带辐条的轮子按走过的距离转，跑动时车身微颠、冒白色尾气、拖速度线，连对氮气尾焰更长、车头微翘；倒数在起点发动机抖，开始发令旗落下起步扬尘，反超司机回头看，还差一分终点旗猛挥，冲过格子线翘头蹦、观众跳起来撒彩纸，输的熄火一股一股冒黑烟。🎈 热气球——从地面浅蓝到高空深蓝的天空、太阳、山丘、飘云，顶上一层厚云是目标；队色球囊带浅色条纹和高光，四根绳吊着藤篮，篮子里探出乘客的脑袋挥手，整只气球绕球顶轻轻摆；得分烧嘴爆燃冒火星、升高一段，连对烧得更旺，倒数停在地上火苗小小的，开始一起爆燃起飞，反超回头看，还差一分云层发光，飞到云上再往上飘、连撒三轮彩纸，输的火熄了、球囊瘪一点往下沉。

- **跟谁打**：三张卡各画一幅示意图（一台手机左边是人右边是机器人 / 一台手机左红队右蓝队 / 两台手机各一队），一眼看出模式。「打机器人」——机器人有自己的题，按 🐢 慢 / 🐰 中 / 🚀 快三档节奏答题，会一个一个按出数字、也会答错，还有表情（🤔 想题、🤖 在按、😄 答对、😅 答错）；「两人一台」——一台 iPad / 电脑横着放，左右各一个人的题和键盘，可以同时按。「各用各的」——每人一台设备，进同一个房间（见下一条）。 设置页默认只有这三张卡和「开始」，选中哪张卡，下面出一行说明；机器人快慢（默认中）、选游戏、改名字都在页头「⚙️ 配置」里。游戏按章节排定：一册里的知识点按目录顺序轮流用这几种游戏，换章节就换游戏，排完从头再排；「⚙️ 配置」里可以换一种或「🎲 随机」，只算这一次，下次又回到章节的游戏。
- **各用各的（多设备房间）**：一个人在设置页选「各用各的」→「建房间」，屏幕上就是三个二维码——左边红队、右边蓝队、下面观战，每个带口令（6 位数字，在二维码上方）、链接和「复制链接」，一句说明「扫码进入、复制链接后打开，或者点顶栏的「加入对战」输口令」，别的没有；建房的这台设备只观战。红队的人扫红队码、蓝队的人扫蓝队码进来，不方便扫码就点顶栏的「🔑 加入对战」输那个身份的口令（没扫过码的设备先问一次名字），先看到一个「三方连接状态」窗口：红队、蓝队各有谁、观战几人，没到的一方写「等待连接…」带加载动画；**红蓝两队都有人进来就自动开始**，不用谁点开始。二维码页上每个码下面也写着谁已进入；一队有人进来后，建房的这台设备也可以点「以另一队进入」自己上场（进去就自动开始），或点「以观战方进入」到连接状态窗口只看。比赛时每台设备上都是完整的竞技场，**只有自己那一行能操作**，别人的行实时显示他在答什么、按了什么、对还是错，队伍比分 = 队里所有人答对的题数之和（每队最多 6 人，人数不要求相等），用时按服务器时钟算。题目在各自设备上生成与判分（同一个种子，服务器只记谁答到第几题、对了几题），断线自动重连并接回原座位（页面回到前台立刻重连），一台设备只算一个人（同一个浏览器再开一个标签页会顶掉先开的）；主持人（建房的设备）掉线 20 秒内回来不换人，超过才把主持权给最早在线的人，主持人退出则立刻交接。结果页三个角色一模一样：「下一章」「再来一局」「不玩了」，谁先点就按谁的、三台设备一起变：「下一章」在同一个房间接着打这一册的下一个知识点（游戏换成那一章的），不用重新建房、重新扫码，可以一直打到这一册最后一个知识点；「不玩了」关掉房间、大家一起回地图。游戏在建房时按设置页定下，线上没有「换个游戏」。房间只在服务器内存里（没人 10 分钟或建房 3 小时后销毁），不落库、不记内容日志；链接 `#/battle/<房间号>?t=red|blue|watch`，房间号本身就是密钥。用 `file://` 打开或托管方没有中继服务时这一项置灰，连不上时提示检查网络。
- **竞技场一律横向**：左区红队、右区蓝队、游戏画面在上方横条或左右之间的竖条（皮肤说了算）。两队一眼分清：队区整块队色描边，队名条是实心队色白字，成员行淡队色底；题目里的按钮（选项卡、数字键）白底、队色描边、队色底边，像立体的键，和底色分得开；横条里的赛跑、赛车每条赛道起点有队色圆牌，竖条游戏红左蓝右和队区对齐。自己那一队的队名条上标「我」，本机不能操作的行蒙一层半透明遮罩、右上角写「对方」或「队友」（两人一台两边都是自己，不标不蒙），只观战的设备顶栏写「👀 观战中」。手机竖着拿会提示「请把手机横过来」，横屏用紧凑版布局（题干与键盘左右并排）；平板 / 手机进竞技场会试着全屏（电脑不会）。
- **名字**：从来没输过名字的设备，第一次点「开始」才问一次「你叫什么？」，可以点现成的（🐰 小兔、🐯 小虎……），以后不再问；两人同屏时右边第一次也问一次，之后各自记住；改名字在设置页页头的「⚙️ 配置」里。名字只显示、不朗读，播报用「红队 / 蓝队」。
...
- **结果页**：🏆 谁赢、比分、用时、每人「答 n · 对 m」，输的一方写「差一点点！」；三种模式都是「下一章」（最大，接着打本册下一个知识点、游戏换成那一章的，最后一个知识点写「这一册都打完啦！」）、「再来一局」（同样的人和游戏，换一组题）、「不玩了」（回地图）；多设备房间里三个角色都一样、谁先点算谁的，「下一章」在同一房间换、「不玩了」关房间大家一起回地图。想换游戏在设置页「⚙️ 配置」里选。对战不改地图上的练习进度，也不存战绩。
...
- **对战怎么玩**：地图右上角打开「⚔️ 对战」再点知识点（或练习页页头的 ⚔️）→ 选「打机器人」（🐢 慢 / 🐰 中 / 🚀 快）、「两人一台」或「各用各的」（建房间出三个二维码和口令，红蓝各扫各的或输口令，人齐了自动开始）→ 游戏按章节排定（配置里可换，只算这一次）→ 第一次问一次名字 → 先讲一句这个游戏的规则 → 预备、3、2、1、开始。竞技场一律横向：左红右蓝，游戏画面在上方横条或中间竖条，每人一行题目 + 键盘 / 选项卡。结束进结果页：下一章 / 再来一局 / 不玩了（三种模式一样；房间里谁点都算）。
...
- **游戏技巧**：先听清题再答（🔊 再听一遍）；数字键盘打错用 ⌫、确认再按 ✓，选项卡先看拼音；打机器人先选 🐢 慢，赢了再升；稳比快重要，连对有加成；两人一台各管各的；各用各的扫码进来等人齐自动开始、掉线别关页面会自动接回；先在练习模式做熟再对战；装到主屏幕全屏、离线都能玩。
- **常见问题**：没声音（看 🔇、iPhone 静音键、先点一下屏幕）、手机要横屏、怎么装到主屏幕离线用、「各用各的」连不上或进不去房间（要网络和对战服务、链接 / 版本 / 一台设备一个人 / 比赛已开始只能观战）、进度只在本机不同步、题目几乎不重复、机器人有三档且会答错、改名字在对战设置页、其它年级和学科陆续补充、免费无广告开源。
...
│   │                        names.ts 现成名字池；sfx.ts WebAudio 合成的音效；skins/ 皮肤注册表（id、位置、类别、canvas 游戏加载函数）；
│   │                        socket.ts 与中继服务的连接（同源 /ws、断线退避重连、心跳、连上前先攒着的消息）
...
│   ├── battle/              对战：竞技场 Arena（三块布局 + 倒数 / 提示 / 胜利 / 结果 / 退出确认，单设备与房间共用，只读 store 不知道自己在哪种模式下）、
│   │                        队区 TeamPanel（进度点、+1、闪光）、成员行 PlayerRow（可操作 / 观看两种，🔥 连对，📶 掉线）、作答显示 WatchInput（机器人表情）、
...
├── views/                   选学科 → 选年级 → 知识点地图 → 练习；battle/ 对战设置页、单设备竞技场页、多设备房间页（二维码页 / 连接状态 → 竞技场 → 结果）
├── stores/                  progress（按知识点记已完成、当前这一轮的 seed 与对错数）、settings（语言 / 声音）、install（安装提示：事件、静默期）、
│                            battle（昵称等偏好 `tongbulian:battle`、当前这一局、答完的反馈窗口、机器人计时器；多设备时状态来自服务器快照、操作发给服务器）、
│                            room（多设备房间：连接、房间快照、我是谁 / 是不是主持人、错误；进队由链接定、开始由服务器自动，客户端只发离开）
...
数据层级：学科 → 年级 → 课程（内容包）→ 上册 / 下册 → 单元 → 知识点 → 生成器。路由 `/s/:subjectId/g/:gradeId/practice/:kpId`；对战只带知识点：`/battle/new/:kpId`（设置）、`/battle/local/:kpId?mode=ai|duo`（单设备竞技场）、`/battle/:code`（多设备房间：二维码页 / 连接状态 → 竞技场 → 结果，`?t=red|blue|watch` 是扫的哪个码）。
...
- **对战与内容解耦**：游戏只吃 `{ red, blue, target, phase, winner, lastPoint }` 和瞬时事件，不认识题目；题目流由 `(kpId, seed, 难度, 序号)` 确定，所以对手 / 观战 / 重连都能复现同一道题，中继服务不用传题；比赛状态机是纯函数，单设备在页面里跑，多设备时同一份跑在中继服务上（服务器只管房间、座位、就绪、谁答到第几题、比分与胜负，客户端判分后报结果；服务器发整份房间快照，每房间每秒最多 10 次，瞬时事件单独发）。
