...
同一个知识点的题，🔴 红队和 🔵 蓝队各答各的，**谁先答对 8 题谁赢**；答错不扣分、不锁题，1 秒多显示正确答案就出下一题。得分不只是数字：每答对一题，游戏画面就发生一次看得见的变化——现在有五种「皮肤」，选游戏时可以挑，也可以「🎲 随机」：

| 皮肤 | 位置 | 得 1 分 | 到 8 分 |
|---|---|---|---|
| 🐢🐇 赛跑 | 上方横条 | 自己的动物向终点跑一格 | 冲线 |
| 🚀 火箭升空 | 左右之间 | 火箭升高一段、喷火 | 到达星星 |
| 🧱 盖楼 | 左右之间 | 加一层 | 封顶放旗、放烟花 |
| 🪢 拔河 | 上方横条 | 绳结向自己这边移一格（位置 = 比分差，翻盘感强） | 对方被拉过线 |
| 🧊 融冰 | 左右之间 | 对方的 8 块冰化掉一块 | 对方冰全化，企鹅滑下来 |

- **跟谁打**：「🤖 打机器人」——机器人有自己的题，按 🐢 慢 / 🐰 中 / 🚀 快三档节奏答题，会一个一个按出数字、也会答错，孩子看得见它在想；「👫 两人一台」——一台 iPad / 电脑横着放，左右各一个人的题和键盘，可以同时按。「📱 各用各的」（每人一台设备、经房间链接同步，可观战）在做，敬请期待。
...
- **声音**：打机器人时进题自动读题（两人同屏不自动读，点 🔊 读自己的题，互相打断）；答对「叮」、答错「咚」、倒数「嘀嘀嘀 — 嘟」、胜利小号声，都是 WebAudio 现场合成的，没有音频文件。
...
├── battle/                  对战：protocol.ts 一局的快照与事件；match.ts 比赛状态机（纯函数：加分、判胜）；stream.ts 题目流
...
│   ├── battle/              对战：队区 TeamPanel、成员行 PlayerRow（可操作 / 观看两种）、作答显示 WatchInput、倒数、结果页、横屏提示、昵称面板、选皮肤
