...
- **跟谁打**：三张卡各画一幅示意图（一台手机左边是人右边是机器人 / 一台手机左红队右蓝队 / 两台手机各一队），一眼看出模式。「打机器人」——机器人有自己的题，按 🐢 慢 / 🐰 中 / 🚀 快三档节奏答题，会一个一个按出数字、也会答错，还有表情（🤔 想题、🤖 在按、😄 答对、😅 答错）；「两人一台」——一台 iPad / 电脑横着放，左右各一个人的题和键盘，可以同时按。「各用各的」——每人一台设备，进同一个房间（见下一条）。
- **各用各的（多设备房间）**：一个人在设置页选「各用各的」→「建房间」，屏幕上立刻出一个大二维码和 6 位房间号（自己已经在红队），其他人用手机扫码就进来了——不用选队，自动分到人少的队（一样多进红队），大厅里可以换队或「只观战」；没法扫码的设备在首页底部「有房间号？加入对战」或对战设置页里输房间号。进来的人看到的是一块自己队色的牌子「你在 🔵 蓝队 · 等主持人开始…」和一个大按钮「🙋 我准备好了」（举手：主持人的名单上出 ✓，顺手把声音解锁；不是开始的条件），建房的人（主持人）看到**两队都有人**就能按底部固定的「开始比赛」。大厅还显示知识点、游戏（主持人可换）、这个游戏的规则句、两队名单（每队最多 6 人，人数不要求相等，谁是主持人、谁掉线了都标着）和观战的人；让子（主持人给每个人各选一档）、锁定队伍、指定队伍的链接（`?t=red|blue|watch`）都收在「更多」里。开始时刻由服务器定，各台设备一起倒数；比赛时每台设备上都是完整的竞技场，**只有自己那一行能操作**，别人的行实时显示他在答什么、按了什么、对还是错，队伍比分 = 队里所有人答对的题数之和，用时按服务器时钟算。题目在各自设备上生成与判分（同一个种子，服务器只记谁答到第几题、对了几题），断线自动重连并接回原座位（页面回到前台立刻重连），一台设备只算一个人（同一个浏览器再开一个标签页会顶掉先开的）；主持人掉线 20 秒内回来不换人，超过才把主持权给最早在线的人，主持人退出则立刻交接。结果页只有主持人有「再来一局 / 换个游戏」（换个游戏 = 大家回大厅，主持人挑完再开始），别人等着。房间只在服务器内存里（没人 10 分钟或建房 3 小时后销毁），不落库、不记内容日志；地址栏 `#/battle/<房间号>`，房间号本身就是密钥。用 `file://` 打开或托管方没有中继服务时这一项置灰，连不上时提示检查网络。
- **竞技场一律横向**：左区红队、右区蓝队、游戏画面在上方横条或左右之间的竖条（皮肤说了算）。两队一眼分清：队区整块队色描边，队名条是实心队色白字，成员行淡队色底；题目里的按钮（选项卡、数字键）白底、队色描边、队色底边，像立体的键，和底色分得开；横条里的赛跑、赛车每条赛道起点有队色圆牌，竖条游戏红左蓝右和队区对齐。手机竖着拿会提示「请把手机横过来」，横屏用紧凑版布局（题干与键盘左右并排）；iPad / Android / 电脑进竞技场会试着全屏。
- **让子**：设置页「更多 · 让子」里每个参赛者各选一档（一档 / 二档 / 三档，默认一档，和练习页一样仍混 15% 高一档）：大人或高年级选三档、小朋友选一档，同一个知识点各答各的档，机器人也按它的档出题（不影响快慢）；行头有「n 档」角标。
