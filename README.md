...
给小学生做的**人教版同步练习**：一到六年级 × 语文 / 数学 / 英语，按**现行最新版人教版教材**（2022 版课标教材）的单元 → 知识点出题，一轮 8 题，做完打勾；还能拿同一个知识点的题**对战**——打机器人、两人一台 iPad，或者每人一台设备扫码进同一个房间。

在线：**[tongbulian.jiaci.app](https://tongbulian.jiaci.app)**（手机 / iPad 打开后添加到主屏幕，离线可用）

- **随机出题，不是题库**：每个知识点一个生成器，题干配十格阵、实物图、钟面、人民币、图形、数轴、尺子、竖式、排队等可视化教具；答错当场用教具演示讲解（凑十法 / 破十法有动画）。
- **每个汉字标拼音，每道题自动朗读**：离线预合成的语音片段拼播，不依赖浏览器 TTS，识字不多的孩子也能自己练。
- **对战模式**：两边比谁先答对 8 题，每答对一题游戏画面就走一步（赛跑、赛车、火箭升空、热气球、盖楼、拔河、融冰），连对 / 反超 / 冲刺有提示和音效，赢了撒彩纸。
- **纯前端、离线、不要账号**：进度只存在本机浏览器里，`dist/index.html` 可以直接双击打开；只有对战的多设备模式需要一个小小的内存中继服务（仓库 `server/`）。
- 界面中文 / 英文一键切换，题目、拼音、朗读都跟着切。

**目前内容**：一年级数学 26 个知识点、二年级数学 29 个知识点全部可练，单元结构与 2024–2026 年新版教材一致（含「数学游戏」和带 ☆ 的综合与实践，清单见[知识点清单](#知识点清单)）；其它年级和语文、英语在目录里占位「敬请期待」，逐步补。
...
4. 想比一比：地图右上角打开「⚔️ 对战」再点知识点（或练习页页头的 ⚔️），选跟谁打，开始。

没有解锁、没有星星、没有排行，不计时、不扣分，想练哪个点哪个；顶部栏的 🔊 / 🔇 可以关掉声音。每个知识点内部分三档难度，一轮里按约 60% 当前档、25% 低一档、15% 高一档混合出题（当前档暂固定为第 1 档，没有更低档，实际 85% / 15%）。

应用里有一页「❓ 帮助与说明」（首页底部、对战设置页页头的「怎么玩」都能进），分学习内容与题目、对战怎么玩、游戏规则、游戏技巧、常见问题五节，中英文随界面切换；在线版另有一张不用 JS 的静态页 [tongbulian.jiaci.app/help/](https://tongbulian.jiaci.app/help/)。

## 对战模式

同一个知识点的题，🔴 红队和 🔵 蓝队各答各的，**谁先答对 8 题谁赢**；答错不扣分、不锁题，1 秒多显示正确答案就出下一题。对战不改地图上的练习进度，也不存战绩。
...
现在有七种游戏，每种都是一个有背景、有一直在动的小装饰、有待机 / 得分 / 冲刺 / 胜负动作的小场景，全部用 canvas 实时绘制（矢量角色、粒子、缓动，没有图片）：
...
**声音**：答对「叮」（连对时越来越高）+ 按游戏一声（赛车轰一下、火箭点火、砖落地咚、融冰咔嚓 + 水滴……）、答错「咚」、倒数「嘀嘀嘀 — 嘟」、弹出提示「啵」、胜利小号声加欢呼，都是 WebAudio 振荡器和噪声现场合成的，没有音频文件；规则句、提示语、播报用的是预合成的朗读片段。
...
- **音频包**：`npm run audio` = `scripts/collect-speech.mjs`（跑遍所有生成器与外壳固定句收集片段 → `src/audio/corpus.json`）+ `scripts/build-audio.py`（edge-tts 合成、裁静音、响度归一、转 24 kHz 48 kbps mp3 → `public/audio/` 与 `src/audio/manifest.json`），现在中英各四百多条、共约 6 MB。有测试保证语料与代码一致、每个片段都有文件。
...
│   └── games/<id>/          每个游戏：model.ts（纯模型，node 可测）、render.ts（只画）、index.ts（组装）——race / car / rocket / balloon / tower / tug / ice
...
├── components/
│   ├── ui/                  通用控件：顶部栏（含「🔑 加入对战」）、页头、大按钮、数字键盘、选择卡、撒花、注音文字 RubyText、安装提示条
...
vite.config.ts               base './'、PWA 清单与预缓存（静态页不进离线包）、siteMeta（按 .env 的 SITE_URL 填绝对地址）、dev 的 /ws 代理
