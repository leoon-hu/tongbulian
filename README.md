...
| 🧱 盖楼 | 左右之间 | 白天的天空、太阳、飘云、飞鸟、山丘与草地，两块地基上各站一个戴队色安全帽的小工人 | 一块带窗户的砖从天上掉下来弹两下、扬起尘土，工人跳到新楼顶；到 6 分楼顶上方一颗星星闪 | 屋顶落下封顶、插旗、窗户逐层亮起、放烟花，工人在屋脊上欢呼 |
| 🪢 拔河 | 上方横条 | 草地、太阳、山丘、小树、中线上的水坑，两头各两只观众，两队各两只穿队色背心的小动物抓着一根粗麻绳 | 绳子正中的蝴蝶结向自己这边弹一截（位置 = 比分差，翻盘感强），自己这队猛拉后仰、对方踉跄打滑扬尘；到 6 分绳子绷紧发抖 | 对方被拖过中线摔进水坑溅起水花，自己这队松手蹦着欢呼、放彩纸 |
| 🧊 融冰 | 左右之间 | 飘雪，底下是水面有鱼游 | 对方的 8 块冰先裂再化成水滴 | 对方冰全化，企鹅滑进水里溅水花 |

手机横屏的矮横条 / 窄竖条会用 CSS 容器查询自动隐藏装饰，只留角色与轨道。

游戏正在逐个升级成 **canvas 实时绘图**（`src/battle/game/` 是与具体游戏无关的部分：契约、自研的 Canvas 2D 小引擎、宿主、保底画面；`src/battle/games/<id>/` 是每个游戏的模型与渲染）。已升级：🐢🐇 赛跑——天空、太阳、飘云、山丘与小树、两条草地跑道各 8 个蘑菇标记（跑过就亮）、举旗的发令员小猴、格子终点柱 + 彩旗拱门 + 4 只观众；乌龟和兔子是代码画的矢量角色（眨眼、待机呼吸、跑步腿摆 / 蹦跳压扁、耳朵后甩、扬尘、速度线），倒数原地蹦、开始落旗迸彩纸、连对加速、反超回头看、还差一分横幅发光、冲线喷彩纸、胜方转圈 / 后空翻、负方缩壳 / 耷拉耳朵。🚀 火箭升空——星空里二十几颗按各自节奏闪的星星、带环的行星、月亮、隔几秒划过的流星，两列各有发射台（平台 + 小塔）、细虚线导轨和顶上带光晕的目标星；火箭是矢量部件（尾翼、队色条纹与鼻锥、舷窗、抖动的尾焰），平时悬浮，得分喷大火向下冒烟升一段，连对火更大；倒数在台上抖、台下冒烟，开始喷火台面闪光，反超被超方晃一下，冲刺目标星脉动，抵达时星星炸成光芒、放三轮队色烟花、火箭绕小圈，负方断火冒黑烟微微下沉。🧱 盖楼——白天的天空、太阳、两层飘云、隔几秒飞过的小鸟、山丘、草地和两块地基；砖是队色圆角砖（底边阴影、顶边高光、按层交替的一扇宽窗 / 两扇窗），每座楼顶站一个矢量小工人（圆脸、队色安全帽、背心、锤子；待机抡锤、隔几秒眨眼），得分一块新砖从天上掉下来弹两下、扬尘、整座楼被压一下再弹回、工人跳上新楼顶，比分一次跳几层就逐块错开落，连对砖掉得更急；倒数工人在地基上蹦，开始举锤欢呼，反超被超的工人挠头看过去，冲刺楼顶上方一颗星星闪，封顶时屋顶从天上落下、队旗挥动、窗户从下往上亮成暖黄、放三轮队色烟花、工人跳上屋脊欢呼，负方工人在没盖完的楼顶坐下歇着。🪢 拔河——草地、太阳、飘云、山丘、两头的小树，正中一条白色虚线的中线和一个水坑，两头各两只观众小动物（左边红队粉丝、右边蓝队粉丝）；两队各两只矢量小动物（熊 + 猪对熊猫 + 猴）穿队色背心、系队色头带，双手抓着一根粗麻绳后仰用力（脚蹬地、屁股后坐、肩膀更靠后、用力时眯眼咬牙），绳子正中一个黄色蝴蝶结，**位置 = 比分差**：得分时整套人和绳子带一点过冲弹过去、蝴蝶结抖一下，被拉的一方踉跄前倾、脚下打滑扬尘，粉丝挥手；倒数四只原地蹦、绳子松垮，开始一起用力绳子绷直，反超被超的队冒汗，冲刺绳子绷紧发抖、观众一直挥手，还差一分蝴蝶结发光；结束时负方两只被拖过中线摔进水坑（溅水花、荡涟漪、坐在水里一脸惊讶），胜方松手举双手蹦着欢呼、放队色彩纸、观众跳起来，绳子落到地上。升级后的游戏仍然只看比分、不认识题目，并且被严格隔离：只能填满竞技场分给它的那个盒子、不接触摸、不出声、没有文字也没有 CSS、按需加载、出错自动换成保底进度条，页面其余部分与布局不受任何游戏改动影响（有测试保证换任何皮肤盒子之外的 DOM 完全一致）。
