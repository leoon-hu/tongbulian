...
游戏正在逐个升级成 **canvas 实时绘图**（`src/battle/game/` 是与具体游戏无关的部分：契约、自研的 Canvas 2D 小引擎、宿主、保底画面；`src/battle/games/<id>/` 是每个游戏的模型与渲染）。已升级：🐢🐇 赛跑——天空、太阳、飘云、山丘与小树、两条草地跑道各 8 个蘑菇标记（跑过就亮）、举旗的发令员小猴、格子终点柱 + 彩旗拱门 + 4 只观众；乌龟和兔子是代码画的矢量角色（眨眼、待机呼吸、跑步腿摆 / 蹦跳压扁、耳朵后甩、扬尘、速度线），倒数原地蹦、开始落旗迸彩纸、连对加速、反超回头看、还差一分横幅发光、冲线喷彩纸、胜方转圈 / 后空翻、负方缩壳 / 耷拉耳朵。🚀 火箭升空——星空里二十几颗按各自节奏闪的星星、带环的行星、月亮、隔几秒划过的流星，两列各有发射台（平台 + 小塔）、细虚线导轨和顶上带光晕的目标星；火箭是矢量部件（尾翼、队色条纹与鼻锥、舷窗、抖动的尾焰），平时悬浮，得分喷大火向下冒烟升一段，连对火更大；倒数在台上抖、台下冒烟，开始喷火台面闪光，反超被超方晃一下，冲刺目标星脉动，抵达时星星炸成光芒、放三轮队色烟花、火箭绕小圈，负方断火冒黑烟微微下沉。升级后的游戏仍然只看比分、不认识题目，并且被严格隔离：只能填满竞技场分给它的那个盒子、不接触摸、不出声、没有文字也没有 CSS、按需加载、出错自动换成保底进度条，页面其余部分与布局不受任何游戏改动影响（有测试保证换任何皮肤盒子之外的 DOM 完全一致）。
